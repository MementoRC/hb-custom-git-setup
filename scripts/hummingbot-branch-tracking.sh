...
# -----------------------------------------
# Starts ci-base from the ci-base-seed-v4 tag (a fixed, GPG-signed snapshot
...
build_ci_base_from_seed() {
    ...
    local seed_tag="ci-base-seed-v4"
    local seed_sha
...
###############################################################################
# Deterministically install one merge stage's content for a conflicted path.
# `git checkout --ours/--theirs` is a silent no-op when the requested stage is
# absent (add/add, modify/delete). With stderr suppressed and $? untested, that
# left the CONFLICTED working-tree copy in place for `git add` to stage as if it
# were a resolution -- markers and all. Writing the stage blob directly cannot
# half-succeed: stage blobs never contain conflict markers.
#   $1 = stage (2 = ours, 3 = theirs), $2 = path
take_merge_stage() {
    local stage="$1" file="$2" tmp rc
    tmp="$(mktemp)" || return 1
    if git show ":${stage}:${file}" > "$tmp" 2>/dev/null; then
        mv "$tmp" "$file"; rc=0
    else
        rm -f "$tmp"; rc=1
    fi
...
merge_for_ci_branches() {
    ...
    while IFS= read -r branch; do
        ...
        if git merge --no-ff "$ref" -m "Auto-merge $branch into $base_branch" > "$merge_log" 2>&1; then
            ...
            local resolution_failed=() marker_files=()
            ...
            for f in "${format_only[@]}"; do
                if [ -n "$merge_base" ] && ! git diff --quiet "$merge_base" "$ref" -- "$f" 2>/dev/null; then
                    ...
                    if [ "$bo_discard_count" -eq 0 ] 2>/dev/null; then
                        take_merge_stage 3 "$f" || resolution_failed+=("$f")
                    ...
                    take_merge_stage 2 "$f" || resolution_failed+=("$f")
            ...
            # A "resolution" that still contains conflict markers is not one. Without
            # this guard `git add` stages the markers, the commit succeeds, and the
            # breakage surfaces ~40 lines later as "FATAL: PY312 TRANSFORM produced
            # undefined names or invalid syntax", misattributing a merge defect to the
            # transform and sending the reader to Pass 1.6. Fail here, where it happened.
            for f in "${format_only[@]}"; do
                if [ -f "$f" ] && grep -qE '^(<<<<<<< |>>>>>>> )' "$f" 2>/dev/null; then
                    marker_files+=("$f")
                fi
            done
            if [ ${#resolution_failed[@]} -gt 0 ] || [ ${#marker_files[@]} -gt 0 ]; then
                if [ ${#resolution_failed[@]} -gt 0 ]; then
                    log_error "Could not extract a merge stage for: ${resolution_failed[*]}"
                fi
                if [ ${#marker_files[@]} -gt 0 ]; then
                    log_error "Conflict markers survived format-only resolution for $branch: ${marker_files[*]}"
                fi
                log_merge_failure "$merge_log" "$conflict_files"
                FAILED_MERGES+=("$branch -> $base_branch")
